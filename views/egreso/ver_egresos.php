  ...
  <div class="container-fluid d-flex justify-content-center align-items-center flex-column mt-4 pt-5 mb-5">
    <div class="row w-100 mb-3 flex-column">
      ...
      <div class="row">
        <div class="col d-flex justify-content-center align-items-center">
          <h1 class="text-primary text-center">Lista de gastos</h1>
        </div>
      </div>
    </div>
    <div class="table-responsive table-responsive-md">
      <table id="tabla" class="table table-hover table-sm">
        ...
        <tbody>
          <?php foreach ($arrayEgreso as $key) { ?>
          <tr>
            <td class="align-middle"><?= $key -> getFechaGasto() ?></td>
            <td class="align-middle"><?php echo $key -> getConceptoGasto() ?></td>
            <td class="align-middle"><?=$key -> getValorGasto() ?></td>
            <td class="align-middle"><?= $key -> getObservacionGasto() ?></td>
            <?php if($nombreRol == 'ADMIN') { ?>
            <td class="text-center align-middle"><a class="btn btn-sm btn-primary m-1 "
                href="EgresoController.php?fn=update&id=<?= $key -> getIdGasto();?>">Actualizar</a><a
                class="btn btn-sm btn-warning m-1"
                href="EgresoController.php?fn=update&id=<?= $key -> getIdGasto();?>">Eliminar</a>
            </td>
            <?php } ?>
          </tr>
          <?php } ?>
